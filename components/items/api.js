...
ItemsAuth.post('/', (req, res) => {
  const {title,type,estimation,linked_items,url,project_id,linked_optional_items,quiz_answers}= req.body;
    ...
    if (title && type && estimation && linked_items) {
        ...
        if (url) {
          if(!validation.validateUrl(url))  return res.send({success:false,msg:'Not valid url'}).status(Settings.HTTPStatus.PARAMS_INVALID);
        }
        if (quiz_answers) {
          if (!validation.validateAnswerAsObject(quiz_answers)) return res.send({success:false,msg:'Not valid Answer'}).status(Settings.HTTPStatus.PARAMS_INVALID);
        }
        items.create({
            title: title,
            type: type,
  ...
  const {url}= req.body;
    if (url) {
      ...
      items.create({
        ...
        if (err) return res.send({success:false,msg:'Server error'}).status(Settings.HTTPStatus.INTERNAL_SERVER_ERROR);
        else res.send({success:true,msg:'url added'}).status(Settings.HTTPStatus.OK);
      });
    }
});
